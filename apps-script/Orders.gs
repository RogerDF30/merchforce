...
function fnOrderList_(p) {
  var email = '';
  if (p.session && validSession_(p.session)) {
    try {
      email = Utilities.newBlob(Utilities.base64DecodeWebSafe(String(p.session).split('.')[0]))
        .getDataAsString().split('|')[0];
    } catch (e) { email = ''; }
  }
  if (!email) return err_('Sign in to see your orders');
  var mine = readRows_('Requests').filter(function (r) {
    return String(r.email).toLowerCase() === email.toLowerCase() ||
           String(r.user_email).toLowerCase() === email.toLowerCase();
  });
  var out = mine.map(function (r, i) {
    return orderPayload_({ rowNum: 0, rec: r }, true);
  }).reverse();
